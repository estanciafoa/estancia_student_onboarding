...
5. **Image cells** (`{{..._photo}}`, `{{..._sig}}`, `{{owner_sig}}`): put the token
...
| `{{owner_sig}}` (image) | owner signature (admin tablet) |
...
| `{{sN_sig}}` (image) | student signature (admin tablet) — also the page-3 signature grid |
